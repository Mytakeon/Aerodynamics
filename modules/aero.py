...
def force_shell(Data, chord, half_span, height, Velocity, thickness=0,
    ...
    if thickness == 0:
        Data['Force'] = [(Cp * 0.5 * air_density * Velocity ** 2 +
                                        atm_pressure) * chord * half_span for Cp in Data['Cp']]
        Data['x'] = [(chord) * x for x in Data['x']]
        Data['y'] = [(chord) * x for x in Data['y']]
    else:
        Data['Force'] = [(Cp * 0.5 * air_density * Velocity ** 2 +
                                        atm_pressure) * chord * half_span for Cp in Data['Cp']]
        Data['x'] = [(chord - 2. * thickness) * x + thickness for x in Data['x']]
        Data['y'] = [(chord - 2. * thickness) * x for x in Data['y']]
    Data['z'] = [0] * len(Data['x'])

    PressureDistribution = list(zip(Data['x'], Data['y'], Data['z'], Data['Force']))
...
def pressure_shell(Data, half_span, chord='MAX', air_density=0, Velocity=0,
    ...
    if 'Cp' in list(Data.keys()):
        Data['Pressure'] = [Cp * 0.5 * air_density * Velocity ** 2 * chord for Cp in Data['Cp']]
    if chord == 'MAX':
        chord = max(Data['x'])
